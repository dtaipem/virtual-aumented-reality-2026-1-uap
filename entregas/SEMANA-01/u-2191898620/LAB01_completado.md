...
**Estudiante:** __Rau Bravo Leonaro Cesar__  **Código:** __2191898620__
...
|-------|-----------------|
| Scene View |Es el espacio donde construyes tu mundo. Aquí colocas, mueves y ajustas todos los objetos 3D (como si fuera tu “mesa de trabajo” del proyecto). |
| Game View |Es cómo se ve realmente tu juego cuando lo ejecutas. Es la “vista del jugador”, no la del editor.|
| Hierarchy |Muestra todos los objetos que existen en la escena. Es como una lista ordenada de todo lo que tienes en tu mundo (cámara, luces, modelos, etc.).|
| Inspector |Aquí ves y editas las propiedades de cualquier objeto seleccionado. Por ejemplo: posición, rotación, escala, materiales, scripts, etc.|
| Project |Es donde están todos tus archivos del proyecto: modelos, scripts, texturas, escenas. Básicamente es tu “explorador de archivos dentro de Unity”.|
| Console |Muestra errores, advertencias y mensajes del sistema. Es clave para depurar cuando algo no funciona en el proyecto.|
...
La Scene View es el espacio de edición donde se construye y organiza el mundo del juego. Aquí puedes mover, escalar y posicionar objetos libremente mientras diseñas la escena.
En cambio, la Game View muestra cómo se ve el juego desde la cámara en tiempo real, es decir, la experiencia final del jugador cuando el juego está en ejecución.

2. ¿Qué hace `Time.deltaTime` en el script de rotación?
representa el tiempo que ha pasado entre un frame y otro. Se usa para que la rotación sea independiente del rendimiento del computador, es decir, evita que el objeto gire más rápido en PCs con más FPS. En este caso, hace que la rotación sea suave y constante en el tiempo, sin importar la cantidad de frames por segundo.

3. Si quisieras que la esfera solo rote cuando se presione la tecla "R", ¿cómo modificarías el script?
Se puede usar Input.GetKey(KeyCode.R) para detectar cuando se presiona la tecla. La rotación solo se ejecutaría dentro de esa condición.
---
